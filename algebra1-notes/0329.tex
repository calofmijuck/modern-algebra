...
\defn. Equivalence classes in \(A\) induced from the relation \(\sim_\sigma\) are called the \textbf{orbits of \(\sigma\)}.
...
\pf \\
\note{Step 1} Take \(\sigma \in S_n\), \(\tau \in S_n\) (transposition). Then for \(\sigma\) and \(\tau\sigma\), their number of orbits differ by 1.
\begin{itemize}
...
\defn. \note{Alternating Group} We define the \textbf{alternating group} \(A_n\) as
\[
